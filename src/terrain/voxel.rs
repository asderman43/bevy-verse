use bevy::{asset::RenderAssetUsages, math::vec3, prelude::*, render::mesh::{Indices, PrimitiveTopology}};

use crate::terrain::table::*;
...
impl Voxel {
    ...
    pub fn set(&mut self, position: (usize, usize, usize), real_size: usize, value: i8) {
        // println!(
        //     "{}",
        //     position.0 + position.1 * real_size + position.2 * real_size * real_size
        // );
        self.chunk[position.0 + position.1 * real_size + position.2 * real_size * real_size] =
            value;
    }
    pub fn create_mesh(&self, size: usize, real_size: usize) -> Mesh {
        ...
        for (x, y, z) in XYZ::new(size) {
            ...
            // x,y,z from corner index:
            //println!("{} {} {}", (i & 1) >> 0, (i & 2) >> 1, (i & 4) >> 2);
        }
        let indices_buffer = (0..vertex_buffer.len()).map(|i| i as u16).collect();

        vertex_buffer = vertex_buffer.iter().map(|vert| vec3(vert.x, vert.y, vert.z)).collect();
